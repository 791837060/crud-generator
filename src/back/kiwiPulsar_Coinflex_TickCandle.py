...
def me_pulsar(topic):
    client = pulsar.Client('pulsar://172.21.31.251:6651')
    consumer = client.subscribe(topic, 'test_ul_mac')
    # while True:
    for i in range(1, 10):
        try:
...
# data=me_pulsar('persistent://OPNX-V1/MD-WS/FLINK-OUT-OHLC-60')
data = me_pulsar('persistent://OPNX-V1/MD-WS/FLINK-OUT-OHLC-TICKER')
# data=me_pulsar('persistent://OPNX-V1/FLINK-JOB/ORDERTICK-MARKPRICE-IN')
# data=me_pulsar('persistent://CF-V2/FLINK-JOB/ORDERTICK-MARKPRICE-IN')


# client=pulsar.Client('pulsar://172.21.31.251:6651')
# consumer = client.subscribe('persistent://OPNX-V1/FLINK-JOB/ORDERTICK-MARKPRICE-IN', 'test_ul_mac')
# consumer.unsubscribe
print(data)
