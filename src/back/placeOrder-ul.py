        ...
        async with websockets.connect(args.ws_url) as websocket:
                ...
                while (1==1):
                    ...
                    for i in range(7):
                        ...
                        # api_orders_create = '{"op": "placeorder", "data": {"clientOrderId": 1, "marketCode": "' + market + '", "side": "BUY", "orderType": "LIMIT",
                        # "quantity": 0.001, "timeInForce": "MAKER_ONLY", "price":"' + price + '", "tag": 102}}'
                        ...
                        api_orders_create = '{"op": "placeorder", "data": {"clientOrderId": 1657789032515, "marketCode": "' + market + \
                                            '", "side": "SELL", "orderType": "LIMIT", "quantity": ' + quantity + ', "timeInForce": "MAKER_ONLY", "price":"' + price + '", "tag": 102}}'
                        await websocket.send(api_orders_create)
                        response = await websocket.recv()
                        msg = json.loads(response)
                        print(msg)

async def helloMARKET():
    for i in range(1):
        async with websockets.connect(args.ws_url) as websocket:
                response = await websocket.recv()
                msg = json.loads(response)
                ...
                while (1==1):
                    ...
                    api_orders_create = '{"op": "placeorder", "data": {"clientOrderId": 1657789032515, "marketCode": "' + market + \
                                        '", "side": "SELL", "orderType": "MARKET", "quantity": 1003, "timeInForce": "MAKER_ONLY", "price":"", "tag": 102}}'
                    await websocket.send(api_orders_create)
                    response = await websocket.recv()
                    msg = json.loads(response)
                    print(msg)
...
asyncio.get_event_loop().run_until_complete(helloLIMIT())